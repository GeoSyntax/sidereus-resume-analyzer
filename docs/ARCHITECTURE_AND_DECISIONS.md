...
- **内存兜底**：本地开发、单实例演示够用，零外部依赖，clone 即跑；未配置 `REDIS_URL` 时自动使用。
- 缺点是多实例/实例回收后缓存丢失（见 §7）——这正是无状态接口要解决的问题。
- **生产已接入 Redis**：线上 FC 通过 `REDIS_URL`（Upstash，TLS `rediss://`）启用 Redis，缓存跨实例共享且持久；`/health` 的 `cache` 字段反映**真实连接状态**（ping 成功才报 `redis`，配错则如实报 `memory`），而非仅看是否配置。
...
### 7.3 无状态接口与 Redis 的先后关系 ⭐

两件事都做了，关键是**顺序**：

- **先用架构消除正确性问题**：无状态 `/analyze` 让匹配不再跨请求依赖服务端状态，多实例下天然正确，且**零外部依赖**。即使完全没有 Redis，主流程也不会 404。
- **再用基础设施做性能优化**：生产接入 Redis（Upstash，TLS）后，缓存跨实例共享且持久——同一份简历/JD 的重复请求命中缓存，`/analyze` 二次调用从约 7s 降到约 0.3s。但缓存**只是性能优化，不是正确性前提**：Redis 挂了自动回落内存，主流程照常工作。
- **决策原则**：先用架构（无状态）消除问题，再用基础设施（Redis）优化，而不是反过来。如果先上 Redis 让两步式流程"能用"，就会把外部依赖变成正确性的前提——方向错了。
...
| 文件 | 覆盖 |
|---|---|
| `test_extractor.py` | 规则抽取必选字段、项目标题不与描述冗余（回归）|
| `test_extractor_cn.py` | 中文简历抽取、姓名跨行 bug 回归 |
| `test_matcher.py` | 技能/经验评分、缺失关键词、关键词同义词去重（回归）|
| `test_pdf_parser.py` | 多页 PDF、扫描件（无文本）异常、非法 PDF |
| `test_api.py` | capabilities、JD 分析、校验错误格式 |
| `test_api_upload.py` | 多页上传、超大文件 413、非 PDF 400、扫描件 422、无状态 `/analyze`（form 传参、<10 字符 422、空白当 parse-only、多页）|
...
| 代码质量 | 25% | 按职责分层、Pydantic 强类型、35 个测试、注释充分、无冗余 |
...
| 加分项 | 10% | 求职意向/薪资/年限/学历/项目抽取、Redis 生产已接入（Upstash/TLS）、LLM 增强、视觉 OCR、示例模式、智能 API Base |
...
- 评分权重是启发式，未经数据回归。
- 缓存 TTL 固定 1 天，未做按需失效/主动刷新（生产已接入 Redis 跨实例共享，无 Redis 时回落内存则不跨实例）。
...
2. **缓存策略进阶**：已接入 Redis 共享缓存，下一步做主动失效/预热与「解析一次、多岗位匹配」工作流的显式建模。
...
A: 这正是我重构的重点。两步式流程依赖内存缓存跨请求存活，但 FC 多实例 + 实例回收会导致第二步落到没有缓存的实例上 404。我加了无状态的 `/api/v1/analyze`，一次调用内完成解析+匹配，缓存退化为纯优化。**顺序很关键**：先用架构（无状态）消除正确性问题，再接 Redis（生产已用 Upstash/TLS）做性能优化——而不是反过来用 Redis 让脆弱的两步流程"勉强能用"。这样即使 Redis 挂了，主流程也不会 404。
